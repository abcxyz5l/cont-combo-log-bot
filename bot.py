...
async def restart_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Admin-only: clear all persisted data (downloads, hits, results, keywords.json).
    This does NOT restart the process, but clears storage so bot starts fresh.
    """
    ...
    deleted_count = 0
    for base in (BASE_DOWNLOAD_DIR, BASE_RESULTS_DIR, BASE_HITS_DIR):
        try:
    ...
    await update.message.reply_text(f"🔁 Restart/Reset complete. Removed ~{deleted_count} files and cleared keywords.")
...
async def download_and_extract(update: Update, context: ContextTypes.DEFAULT_TYPE, url: str, link_num: int, total_links: int, user_id: int):
    ...
    try:
        ...
        if count > 0:
            ...
            await safe_edit(progress_message, f"✅ **[{link_num}/{total_links}]** Found {count} hits! Sending now...")

            # Send the hit file immediately to the user, then delete both raw download and hit file
            try:
                with open(final_path, "rb") as f:
                    await update.message.reply_document(
                        document=InputFile(f, filename=os.path.basename(final_path)),
                        caption=f"📋 Hits from link {link_num}/{total_links} — {count} entries",
                    )
            except Exception:
                # If sending fails, keep the hit file and report
                await safe_edit(progress_message, f"⚠️ **[{link_num}/{total_links}]** Found {count} hits but failed to send file")
            else:
                # On successful send, remove raw download but KEEP the hit file so /view and /send work
                try:
                ...
                # Ensure hits list exists and append this hit file for later /view and /send
                try:
                    context.user_data.setdefault("hits_files", [])
                    context.user_data["hits_files"].append((final_path, count))
                except Exception:
                    pass

                await safe_edit(progress_message, f"✅ **[{link_num}/{total_links}]** Sent {count} hits. Hit file kept for /view and /send.")
        else:
        ...
        try:
            ...
            if ok:
                ...
                if count > 0:
                    ...
                    if progress_message:
                        await safe_edit(progress_message, f"✅ **[{link_num}/{total_links}]** Found {count} hits (curl)! Sending now...")

                    try:
                        with open(final_path, "rb") as f:
                            await update.message.reply_document(
                                document=InputFile(f, filename=os.path.basename(final_path)),
                                caption=f"📋 Hits from link {link_num}/{total_links} — {count} entries (curl)",
                            )
                    except Exception:
                        if progress_message:
                            await safe_edit(progress_message, f"⚠️ **[{link_num}/{total_links}]** Found {count} hits but failed to send file (curl)")
                    else:
                        try:
                            if os.path.exists(dest_path):
                                os.remove(dest_path)
                        ...
                        if progress_message:
                            await safe_edit(progress_message, f"✅ **[{link_num}/{total_links}]** Sent {count} hits (curl). Hit file kept for /view and /send.")
                else:
                    if progress_message:
                        await safe_edit(progress_message, f"⚠️ **[{link_num}/{total_links}]** No hits for your keywords")
                    try:
...
async def process_links_batch(update: Update, context: ContextTypes.DEFAULT_TYPE, links: list):
    ...
    async def background_download():
        """Run in background sequentially. Stops when user sends /stop (task cancelled)."""
        try:
            total_found = 0
            processed = 0
            for idx, url in enumerate(links, 1):
                ...
                try:
                    ...
                    try:
                        await update.message.reply_text(f"⚠️ Error processing link {idx}. Skipping to next.")
                    except Exception:
                        pass

            if context.user_data.get("stop_requested"):
                try:
                    await update.message.reply_text("⏹ Processing stopped by user.")
                except Exception:
                    pass
                return

            try:
                await update.message.reply_text(f"✅ Processed {processed}/{len(links)} links. Finished.")
            except Exception:
                pass
        except asyncio.CancelledError:
            pass
...
def main():
    ...
    app.add_handler(CommandHandler("reset", restart_cmd))
